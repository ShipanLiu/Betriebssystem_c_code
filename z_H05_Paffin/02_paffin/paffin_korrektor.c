...
int main(int argc, char** argv) {
  ...
  for(int i=3; i<argc; i++) {
    errno = pthread_create(&tids[i], NULL, processTree, argv[i]); //因为 argv 里面存的就是 string
...
/*I----> +--------------------------------------------------------------------+
         | Du müsstest hier die errno vor dem ersten Aufruf von readdir()    |
         | oben auf 0 setzen, da readdir() ja die errno setzt und du mit      |
         | errno = 0 danach dann die errno mit 0 überschreiben würdest.     |
         | Also so die errno setzen, wie du es unten in der while Schleife    |
         | getan hast (-0.5)                                                  |
         +-------------------------------------------------------------------*/
...
/*I----> +--------------------------------------------------------------------+
         | errno kann mit dem Aufruf von V() vor perror überschrieben        |
         | werden. Solange in der Schnittstelle (hier sem.h für V()) nicht   |
         | explizit drinsteht, dass die errno nicht überschrieben/geändert  |
         | wird, muss davon ausgegangen werden, dass die errno überschrieben |
         | werden kann (siehe auch Korrekturhinweise Moodle, Abschnitt        |
         | Programmmierfehler) (-0.5). Du müsstest hier also die errno       |
         | vorher zwischenspeichern und nach dem Aufruf von V() wieder        |
         | zurücksetzen.    忽略这个警告                                                 |
         +-------------------------------------------------------------------*/
...
/*I----> +--------------------------------------------------------------------+
         | Hier ist die Reihenfolge des Semphoren Freigebens entscheidend. In |
         | eurer Variante könnte es passieren, dass das Aufräumen/Beenden   |
         | des Programms nach V(mutex) schon mal loslegt und eure Semaphore   |
         | schonmal mit semDestroy() wegräumt, bevor hier V() auf die beiden |
         | aufgerufen wurde. Damit habt ihr eine Race Condition. (-1.0).      |
         +-------------------------------------------------------------------*/

  // // one thread dead --> unblock the main, check if the number of active threads is 0 or not.
  // V(semNotify);
